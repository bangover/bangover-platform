...
import cloud.bangover.platform.domain.functions.search.query.data.MockSearchDataStore;
...
public class PaginatedQueryExecutionFunctionTest {
  ...
  private final MockSearchDataStore dataStore = new MockSearchDataStore();
  ...
  public void shouldFilterElementsWithPagination() {
    ...
    BusinessFunction<PaginatedSearchQuery,
        CollectionWrapper<SearchData>> businessFunction = new PaginatedQueryExecutionFunction<>(
            dataStore, query -> new MockSearchDataPaginatedSpec(query.getDiscriminator(),
                query.getPagination()));
