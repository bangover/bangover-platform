...
public class RetrieveEntityFunctionTest {
  ...
  @RunWith(JUnit4.class)
  public static class FailOnEmptyResultTest {
    private MockEntityStore<Long, MockEntity> store = new MockEntityStore<>();
    private BusinessFunctionRegistry registry = new DefaultBusinessFunctionExecutor();
    ...
    @Test
    public void shouldEntityBeRetrieved() throws Exception {
      // Given
      store.loadDataSet(DataSet.of(Arrays.asList(MOCK_ENTITY)));
      BusinessFunction<Long, MockEntity> function =
          new RetrieveEntityFunction<>(store).failOnEmptyResult();
      RequestReplyInteractor<Long, MockEntity> interactor =
          registry.registerRequestReplyFunction(Long.class, MockEntity.class, function);
      // When

      MockEntity result = interactor.invoke(1L).get(10L);
      // Then
      Assert.assertEquals((Long) 1L, result.getId());
      Assert.assertEquals(DESCRIPTION_REQUEST, result.getDescription());
    }
    ...
    @Test
    public void shouldBeRejectedIfEntityNotFound() {
      // Given
      BusinessFunction<Long, MockEntity> function = new RetrieveEntityFunction<>(store).failOnEmptyResult();
      RequestReplyInteractor<Long, MockEntity> interactor =
          registry.registerRequestReplyFunction(Long.class, Optional.class, function);
      // When
      Promise<MockEntity> result = interactor.invoke(1L);
