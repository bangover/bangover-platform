...
public interface EntityStore<I, E extends Entity<I>>
    extends RetrieveEntity<I, E>, SaveEntity<I, E>, DeleteEntity<I, E> {

  default <C extends Entity<I>> EntityStore<I, C> derive(Function<C, E> forwardEntityConverter,
      Function<E, C> backwardEntityConverter) {
    return derive(id -> id, forwardEntityConverter, backwardEntityConverter);
  }

  default <A, C extends Entity<A>> EntityStore<A, C> derive(Function<A, I> idConverter,
      Function<C, E> forwardEntityConverter, Function<E, C> backwardEntityConverter) {
    EntityStore<I, E> store = this;
    return new EntityStore<A, C>() {
      @Override
      public Optional<C> find(A id) {
        return store.find(idConverter.apply(id)).map(backwardEntityConverter);
      }
