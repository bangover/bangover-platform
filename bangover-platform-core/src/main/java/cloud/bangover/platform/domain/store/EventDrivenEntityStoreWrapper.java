...
public class EventDrivenEntityStoreWrapper<I, E extends EventDrivenEntity<I, E>>
  ...
  public void save(E entity) {
    Collection<EventDescriptor<Object>> publishedEvents = entity.getPublishedDomainEvents();
    super.save(entity);
  ...
  public void delete(I id) {
    Collection<EventDescriptor<Object>> publishedEvents =
        find(id).map(EventDrivenEntity::getPublishedDomainEvents).orElse(Collections.emptyList());
    super.delete(id);
    republishLocallyPublishedEntityEvents(publishedEvents);
  }
  ...
  private void republishLocallyPublishedEntityEvents(
    ...
    entityEvents.forEach(eventDescriptor -> {
      GlobalEvents.publisher(eventDescriptor.getBoundedContext(), eventDescriptor.getEventType())
